...
//随机文字烟花内容
const randomWords = ["HelloWorld","NewYear","Happy","Cat"];
const wordDotsMap = {};
randomWords.forEach((word) => {
	wordDotsMap[word] = MyMath.literalLattice(word, 3, "Gabriola,华文琥珀", "150px");
});
...
function init() {
	...
	// shell size
	options = "";
	['0"','3"', '4"', '6"', '8"', '12"', '16"'].forEach((opt, i) => (options += `<option value="${i}">${opt}</option>`));
